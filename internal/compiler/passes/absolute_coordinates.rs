...
pub fn lower_absolute_coordinates(component: &Rc<Component>) {
    ...
    let point_type = crate::typeregister::point_type();

    for nr in to_materialize {
        let elem = nr.element();

        let absolute_point_prop_name = "absolute-xy".to_string();
        ...
        elem.borrow_mut().bindings.insert(
            ...
            RefCell::new(
                Expression::StructFieldAccess {
                    base: Expression::PropertyReference(NamedReference::new(&elem, "absolute-xy"))
                        .into(),
